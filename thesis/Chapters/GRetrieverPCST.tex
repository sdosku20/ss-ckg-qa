...
% EDITORIAL NOTE. Chapter 2 deliberately describes PCST selection without algebra,
% and promises the detail here exactly once. This chapter is that promise: the
% objective, the prize and cost definitions, the transformation that lets edges
% carry prizes, the solver, and -- Section 3.7 -- what those definitions predict
% on a graph with the properties measured in Section 1.2. Those predictions are
% what Chapter 6 tests, so the analysis here is not decoration: it is the
% hypothesis.

Chapter~\ref{chapter:background} placed PCST-based selection among the
alternatives and deferred the mathematics. This chapter supplies it. The purpose
is not completeness for its own sake but a specific one: the behaviour this
thesis observes on clinical data follows from the prize and cost definitions
rather than from anything clinical, and that can only be shown by writing them
down. Section~\ref{sec:pcst-consequences} derives two predictions from the
definitions alone, and Chapter~6 reports what happened when they were tested.
...
about $26$ nodes per text. The prediction is that similarity identifies the
...
the connectivity constraint depends on paths being expensive. When a small set of
nodes has very high degree -- $168$ provenance nodes of average degree
$69{,}525$, six anatomical-site nodes carrying $3{,}454{,}881$ edges -- almost
any pair of nodes is joined by a path of length two or three through one of them.
...
validated on a different kind of graph. They are the price of the transfer, and
stating them as consequences of Equations~\ref{eq:node-prizes}
to~\ref{eq:cost-adjustment} rather than as observations is what makes them
testable. Chapter~4 turns each into a methodological choice that can be switched
off, and Chapter~6 reports the measurements.
