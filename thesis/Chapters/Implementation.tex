...
Two claims need to be checkable for the results to mean anything: that the PCST
implementation used here behaves exactly like the published one, and that every
number reported later can be regenerated. The software is a Python package,
\texttt{ikgqa}, organised so that each concern raised in
Chapter~\ref{chapter:methodology} lives in one place and can be exercised on its
own, which is what makes both claims testable rather than asserted.
...
\texttt{ikgqa.eval} & the metric, the sweep, the gold set, the cohort loop, the figures \\
\texttt{ikgqa.baselines} & the SnapQuery client, which is a service rather than a retriever \\
\texttt{ikgqa.data} & the Neo4j loader, terminology resolution, the replica \\
\bottomrule
\end{tabular}
\end{table}

Three design decisions in Table~\ref{tab:modules} are load-bearing rather than
stylistic.
...
Every retrieval strategy implements one interface and nothing else: given a graph
and an embedded question, return node and edge identifiers. The strategies do
...
the premise of the entire comparison. SnapQuery is the exception and lives
outside that package: it consumes a question in words and returns rows rather
than a subgraph, so forcing it behind the retriever interface would hide the
adaptation the comparison depends on instead of stating it.

Nodes carry a stable key alongside their index. The key is derived from the SPHN
uid and does not move; the index is assigned by whatever order the loader walked
the database and moves whenever the loader, a limit or the data changes. Without
a key there is nowhere to record a gold answer that survives a reload, and an
answer recorded as an index would keep scoring while denoting a different node.
This is the mechanism behind Section~\ref{sec:methodology-goldset}, and it costs
one column.
...
Fetching and assembly are separate functions. Assembly is pure, taking rows in
and returning a graph, so it is unit tested against fixture rows with no
database, while the
...
distinct texts at eleven per cent high, stays visible rather than buried.
...
The first is the retrieval contract, requiring identifiers to be sorted, unique
and in range, with the node set closed under the returned edges. It is asserted
after every strategy in
