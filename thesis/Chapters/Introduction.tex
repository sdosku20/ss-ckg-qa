...
once, at a cost in time and compute that would be impractical regardless. The
limit is not hypothetical here. The planner model deployed behind the comparator
studied in this thesis serves a context of 8{,}192 tokens, and the live database
schema and the agent's own reasoning are generated inside that same window. One
prepared patient graph, at 45{,}562 edges and on the order of ten tokens per
serialised triple, comes to roughly $4.5 \times 10^{5}$ tokens. Under one per
cent of a \emph{single} patient can be placed in front of that model at once. So
...
for 1{,}197 patients, roughly 18{,}000 nodes per patient. Even after the graph
preparation described in Chapter~\ref{chapter:methodology}, a single patient
comes to 15{,}810 nodes and 45{,}562 edges -- about eleven times the average
WebQSP sample, for one patient out of 1{,}197. Selecting over the whole graph per
question is therefore not merely expensive but structurally different from the
published setting, where the candidate region was fixed in advance and small.

\paragraph{Sparse and shared text.} Almost no node carries text of its own.
Laboratory results account for 20{,}523{,}891 of the graph's nodes -- 95.3\% --
because the schema represents one result as four separate event nodes, and they
are described only through the terminology codes they reference: 4{,}832{,}744
laboratory tests resolve to just 544 distinct LOINC descriptions. The effect
survives preparation. One patient's 15{,}810 prepared nodes carry 614 distinct
descriptions, about 26 nodes per description, so a similarity score can sort that
patient's graph into at most 614 classes. It can identify which \emph{analyte} a
question is about but not which \emph{measurement}.
...
derived from its prize and cost definitions in
Chapter~\ref{chapter:pcst} and then measured in
Chapter~\ref{chapter:results}:
  ...
  nothing: every strategy tested, connected or not, recalled the same amount,
  because on this graph the similarity signal rather than the structural
  strategy decides what is retrieved.
  \end{enumerate}
...
preparation rather than in retrieval -- and reverses entirely when the
...
scale at no cost in recall, reported with the \emph{ceiling} it imposes so that a
narrowing failure cannot be mistaken for a selection failure. It is shown to
depend on terminology resolution having been applied first.
...
positions PCST-based selection among the alternatives.
Chapter~\ref{chapter:pcst} gives the mathematical treatment that
Chapter~\ref{chapter:background} deliberately leaves out, and derives from the
prize and cost definitions three predictions about how the method should behave
on a graph with the properties measured above.
Chapter~\ref{chapter:methodology} sets out the methodology: the graph
preparation procedure, the two-stage retrieval design, the strategies compared,
the evaluation protocol and the question sets.
Chapter~\ref{chapter:implementation} describes the implementation and how its
fidelity to the published one is established.
Chapter~\ref{chapter:results} reports the measurements and tests the three
predictions, and Chapter~7 concludes.
