...
% EDITORIAL NOTE. Every choice in this chapter is stated as a decision with a
% measured justification and an off switch, because a pipeline whose steps cannot
% be disabled cannot be ablated, and an unablatable step cannot be reported as a
% contribution. Where a decision is still open it is marked as such rather than
% quietly settled.

This chapter sets out how the comparison is made: what is done to the graph
before any retrieval happens, how retrieval is staged, what is measured, and on
which questions. The guiding constraint is that each step must be switchable, so
that Chapter~\ref{chapter:results} can attribute an effect to it rather than to
...
\texttt{BodySite}, \texttt{ReferenceRange} -- are removed as nodes and their text
...
and a \emph{display text}. The embedding text carries the semantic core -- what
kind of measurement, which analyte -- and deliberately excludes the measured
...
catalogues to supply descriptions the graph lacks, in five tiers -- exact match,
a different version of the same classification, the parent concept, the graph's
own label, and nothing -- with the count of nodes answered by each tier reported
alongside any result that depends on it. The design is a cross-walk rather than a
...
therefore made against a \emph{measured} size -- node count, or prompt characters
-- with each strategy swept across its own parameter range and the resulting
points placed on a shared axis.
