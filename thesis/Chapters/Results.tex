...
% EDITORIAL NOTE. Every number in this chapter is either measured on the live
% clinical graph or produced by a script in experiments/, and each section names
% which. Section 6.1 draws the line between the two and states plainly what has
% NOT been measured yet, so no later section has to hedge. Do not add a number
% here without a command that reproduces it.

\section{What Is Measured Here, and What Is Not} \label{sec:results-scope}

Results in this chapter come from two sources, and conflating them would make
the chapter worthless.
...
structure of one real patient exactly -- the same node and edge counts, the same
...
evidence about \emph{mechanism} -- whether a size parameter controls size,
whether tie-breaking decides an outcome, whether resolving a code changes what can
be retrieved -- because those depend on structure. They are not evidence about
...
45{,}562 of 45{,}562 edges} -- the whole graph, for every question tested.
...
parameters are not the same quantity -- a cost against a count against a hop
number -- the comparison is made on measured size, as
...
scored $0.164$ -- identically. On this graph, at the sizes PCST operates in, what
...
at one hop and $15{,}334$ at two -- from $0.05\%$ of the graph to $97\%$ with no
...
the code they are looking for -- a question containing \texttt{N18.5} finds the
...
the language users write questions in -- that is not a detail to settle late.
...
recall is a larger subgraph, not a better one -- the clearest illustration in
...
bag-of-words encoder. That is appropriate for structural questions -- a size dial
either controls size or does not, whatever the embedding -- and inappropriate for
